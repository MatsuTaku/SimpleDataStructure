...
const mask256_type const_mask256;

template<> const mask256_type const_mask256<1, 0> = mask256_type("38597363079105398474523661669562635951089994888546854679819194669304376546645");
template<> const mask256_type const_mask256<1, 1> = mask256_type("77194726158210796949047323339125271902179989777093709359638389338608753093290");
template<> const mask256_type const_mask256<2, 0> = mask256_type("23158417847463239084714197001737581570653996933128112807891516801582625927987");
template<> const mask256_type const_mask256<2, 2> = mask256_type("92633671389852956338856788006950326282615987732512451231566067206330503711948");
template<> const mask256_type const_mask256<4, 0> = mask256_type("6811299366900952671974763824040465167839410862684739061144563765171360567055");
template<> const mask256_type const_mask256<4, 4> = mask256_type("108980789870415242751596221184647442685430573802955824978313020242741769072880");
template<> const mask256_type const_mask256<8, 0> = mask256_type("450552876409790643671482431940419874915447411150352389258589821042463539455");
template<> const mask256_type const_mask256<8, 8> = mask256_type("115341536360906404779899502576747487978354537254490211650198994186870666100480");
template<> const mask256_type const_mask256<16, 0> = mask256_type("1766820105243087041267848467410591083712559083657179364930612997358944255");
template<> const mask256_type const_mask256<16, 16> = mask256_type("115790322417210952336529717160220497262186272106556906860092653394915770695680");
template<> const mask256_type const_mask256<32, 0> = mask256_type("26959946660873538060741835960174461801791452538186943042387869433855");
template<> const mask256_type const_mask256<32, 32> = mask256_type("115792089210356248762697446947946071893095522863849111501270640965525260206080");
template<> const mask256_type const_mask256<64, 0> = mask256_type("6277101735386680763495507056286727952657427581105975853055");
template<> const mask256_type const_mask256<64, 64> = mask256_type("115792089237316195417293883273301227089774477609353836086800156426807153786880");
template<> const mask256_type const_mask256<128, 0> = mask256_type("340282366920938463463374607431768211455");
template<> const mask256_type const_mask256<128, 128> = mask256_type("115792089237316195423570985008687907852929702298719625575994209400481361428480");
#endif
...
inline int clz256(mask256_type x) {
    ...
    if (x == 0)
        return 256;
    int c = 0;
    if (x & const_mask256<128, 128>) {
        x &= const_mask256<128, 128>;
        c |= 128;
    }
    if (x & const_mask256<64, 64>) {
        x &= const_mask256<64, 64>;
        c |= 64;
    }
    if (x & const_mask256<32, 32>) {
        x &= const_mask256<32, 32>;
        c |= 32;
    }
    if (x & const_mask256<16, 16>) {
        x &= const_mask256<16, 16>;
        c |= 16;
    }
    if (x & const_mask256<8, 8>) {
        x &= const_mask256<8, 8>;
        c |= 8;
    }
    if (x & const_mask256<4, 4>) {
        x &= const_mask256<4, 4>;
        c |= 4;
    }
    if (x & const_mask256<2, 2>) {
        x &= const_mask256<2, 2>;
        c |= 2;
    }
    if (x & const_mask256<1, 1>) {
        x &= const_mask256<1, 1>;
        c |= 1;
    }
    return c ^ 255;
...
inline mask256_type xor_idx(mask256_type x, uint8_t y) {
    if (y & 1)
        x = (((x&const_mask256<1, 0>) << 1) | ((x&const_mask256<1, 1>) >> 1));
    if (y & 2)
        x = (((x&const_mask256<2, 0>) << 2) | ((x&const_mask256<2, 2>) >> 2));
    if (y & 4)
        x = (((x&const_mask256<4, 0>) << 4) | ((x&const_mask256<4, 4>) >> 4));
    if (y & 8)
        x = (((x&const_mask256<8, 0>) << 8) | ((x&const_mask256<8, 8>) >> 8));
    if (y & 16)
        x = (((x&const_mask256<16, 0>) << 16) | ((x&const_mask256<16, 16>) >> 16));
    if (y & 32)
        x = (((x&const_mask256<32, 0>) << 32) | ((x&const_mask256<32, 32>) >> 32));
    if (y & 64)
        x = (((x&const_mask256<64, 0>) << 64) | ((x&const_mask256<64, 64>) >> 64));
    if (y & 128)
        x = (((x&const_mask256<128, 0>) << 128) | ((x&const_mask256<128, 128>) >> 128));
    return x;
}
#endif
